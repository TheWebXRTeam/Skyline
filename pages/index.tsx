...
const Butterflies = ({
	...
	useFrame((state, delta) => {
		...
		// Grabbing
		// Handle the two handed grabbing of the same object
		if (selectedObjectLeft.current && selectedObjectRight.current) {
		...
		// one hand is grabbing, not the other
		else if (
			...
			// get a reference to the current selected object, regardless of which hand it is in
			// in this case, it can only be in one hand
			const selectedObject = isLeftHand
				? selectedObjectLeft.current
				: selectedObjectRight.current;

			// selectedObject.current.grab();
			selectedObject.current.setTarget(grabbingHand.position, 0)
